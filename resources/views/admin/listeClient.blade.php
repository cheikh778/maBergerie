                ...
                <div class="col-12">
                    <div class="row justify-content-center ">
                        <div class="col-lg-6 text-center ">
                            <h1 class="mb-4 heading text-white" data-aos="fade-up" data-aos-delay="100">Liste des Clients</h1>
    ...
    <div class="container">
        <div class="row">
                ...
                <div class="col-lg-6 mb-4" data-aos="fade-up" data-aos-delay="100">
                    <div class="media-h d-flex h-100">
                        ...
                        <div class="media-h-body">
                            <h2 class="mb-3"><a
                                    href="{{ route('users.show', $client->id) }}">{{ $client->nom }} {{ $client->prenom }}</a></h2>
                            <p>Adresse: {{ $client->adresse }}</p>
                            <p>Profil: {{ $client->role }}</p>
                            <p>Emails: {{ $client->email }} </p>

                            <a href="{{ route('users.show', $client->id) }}" class="btn btn-primary btn-sm">Détails</a>
                            <a href="{{ route('users.destroy', $client->id) }}" class="btn btn-danger btn-sm"
                                onclick="event.preventDefault(); if(confirm('Êtes-vous sûr?')) document.getElementById('delete-form-{{ $client->id }}').submit();">Supprimer</a>

                            <form id="delete-form-{{ $client->id }}"
                                action="{{ route('users.destroy', $client->id) }}" method="POST"
                                style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>&nbsp;
                            <br>
                            @if ($client->status)
                                <form method="POST" action="{{ route('users.block', $client->id) }}">
                                    @csrf
                                    <button type="submit" class="btn btn-secondary btn-sm">Bloquer</button>
                                </form>
                            @else
                                <form method="POST" action="{{ route('users.unblock', $client->id) }}">
                                    @csrf
            ...
            <div class="col-12 text-center">
                {{ $clients->links() }} <!-- Affiche la pagination -->
            </div>
