                ...
                <div class="col-12">
                    <div class="row justify-content-center ">
                        <div class="col-lg-6 text-center ">
                            <h1 class="mb-4 heading text-white" data-aos="fade-up" data-aos-delay="100">Liste des Eleveurs</h1>
    ...
    <div class="container">
        <div class="row">
                ...
                <div class="col-lg-6 mb-4" data-aos="fade-up" data-aos-delay="100">
                    <div class="media-h d-flex h-100">
                        ...
                        <div class="media-h-body">
                            <h2 class="mb-3"><a
                                    href="{{ route('users.show', $eleveur->id) }}">{{ $eleveur->nom }} {{ $eleveur->prenom }}</a></h2>
                            <p>Adresse: {{ $eleveur->adresse }}</p>
                            <p>Profil: {{ $eleveur->role }}</p>
                            <p>Emails: {{ $eleveur->email }} </p>

                            <a href="{{ route('users.show', $eleveur->id) }}" class="btn btn-primary btn-sm">Détails</a>
                            <a href="{{ route('users.destroy', $eleveur->id) }}" class="btn btn-danger btn-sm"
                                onclick="event.preventDefault(); if(confirm('Êtes-vous sûr?')) document.getElementById('delete-form-{{ $eleveur->id }}').submit();">Supprimer</a>

                            <form id="delete-form-{{ $eleveur->id }}"
                                action="{{ route('users.destroy', $eleveur->id) }}" method="POST"
                                style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>&nbsp;
                            <br>
                            @if ($eleveur->status)
                                <form method="POST" action="{{ route('users.block', $eleveur->id) }}">
                                    @csrf
                                    <button type="submit" class="btn btn-secondary btn-sm">Bloquer</button>
                                </form>
                            @else
                                <form method="POST" action="{{ route('users.unblock', $eleveur->id) }}">
                                    @csrf
            ...
            <div class="col-12 text-center">
                {{ $eleveurs->links() }} <!-- Affiche la pagination -->
            </div>
