                ...
                <div class="col-12">
                    <div class="row justify-content-center ">
                        <div class="col-lg-6 text-center ">
                            <h1 class="mb-4 heading text-white" data-aos="fade-up" data-aos-delay="100">Liste des Administrateurs</h1>
    ...
    <div class="container">
        <div class="row">
                ...
                <div class="col-lg-6 mb-4" data-aos="fade-up" data-aos-delay="100">
                    <div class="media-h d-flex h-100">
                        ...
                        <div class="media-h-body">
                            <h2 class="mb-3"><a
                                    href="{{ route('users.show', $admin->id) }}">{{ $admin->nom }} {{ $admin->prenom }}</a></h2>
                            <p>Adresse: {{ $admin->adresse }}</p>
                            <p>Profil: {{ $admin->role }}</p>
                            <p>Emails: {{ $admin->email }} </p>

                            <a href="{{ route('users.show', $admin->id) }}" class="btn btn-primary btn-sm">Détails</a>
                            <a href="{{ route('users.destroy', $admin->id) }}" class="btn btn-danger btn-sm"
                                onclick="event.preventDefault(); if(confirm('Êtes-vous sûr?')) document.getElementById('delete-form-{{ $admin->id }}').submit();">Supprimer</a>
                            ...
                            <br>
                            @if ($admin->status)
                                <form method="POST" action="{{ route('users.block', $admin->id) }}">
                                    @csrf
                                    <button type="submit" class="btn btn-secondary btn-sm">Bloquer</button>
                                </form>
                            @else
                                <form method="POST" action="{{ route('users.unblock', $admin->id) }}">
                                    @csrf
            ...
            <div class="col-12 text-center">
                {{ $admins->links() }} <!-- Affiche la pagination -->
            </div>
